...
import numpy as np
from plastinka_classes import Element, Node
    ...
    for i in range(len(triSimplices)):
        ...
        for j in range(3):
            ...
            if seenNodes.get(triSimplices[i,j]):
                ...
                continue
            elif nodeCoordinates[0] == 0 and (nodeCoordinates[1] <= 20): # right top and bot nodes
                node = Node(triSimplices[i][j], nodeCoordinates, 0, True)
            elif nodeCoordinates[0] == borderX: # right top and bot nodes
...
def calculateQMatrix(globalMatrix, matrixF):
    ...
    return np.linalg.solve(globalMatrix, matrixF) * 1e6
...
def recalculateStress(numberOfNodes: int, force: float, thickness: float , borderX: float, borderY: float, ax: Axes,cax: Axes,fig: Figure, ax1: Axes, ax2:Axes):
    # print(force)
    newPoints = []
    xPoints = np.linspace(0,borderX,numberOfNodes)
    yPoints = np.linspace(0, borderY, numberOfNodes)
    globalMatrix = np.zeros([numberOfNodes*numberOfNodes*2,numberOfNodes* numberOfNodes*2])
    for i in range(xPoints.size):
        for j in range(yPoints.size):
            points = newPoints.append([xPoints[i] , yPoints[j]])
    ...
    line2, = ax2.plot(deformationPoints[:, 0], deformationPoints[:,1], 'o')
    line2.set_data(deformationPoints[:, 0], deformationPoints[:,1])

    tri1 = Triangulation(deformationPoints[:, 0], deformationPoints[:,1])
    ax1.triplot(tri1)
    ax2.triplot(tri1)
    tpc1 = ax1.tripcolor(deformationPoints[:, 0], deformationPoints[:,1],strains,triangles=triangles, cmap="jet")
